...
function loadPrivateKey(): crypto.KeyObject {
  ...
  if (!pemKey) throw new Error("RSA_PRIVATE_KEY_PEM not configured");
...
export async function blindSign(
  ...
  try {
    ...
    const signature = await prisma.$transaction(async (tx:any) => {
      ...
      let status: string = votingToken.poll.status;
      if (votingToken.poll.chainPollId != null) {
        const chainState = await getPollChainState(
          votingToken.poll.chainPollId,
        );
        if (chainState !== null) {
          status = chainStateToStatus(chainState);
        } else {
          console.warn(
            `[blindSign] Chain state unavailable for poll ${votingToken.pollId}, falling back to DB status '${votingToken.poll.status}'`,
          );
        }
      }
