 ...
 */
interface Candidate {
  id: string;       // Candidate ID as string
  name: string;     // Candidate name
}

/**
 * Interface representing a vote with ranked preferences
 */
interface VoteRanking {
  voteId: BigInt;                              // Unique ID of the vote
  rankings: { candidateId: BigInt; rank: number }[]; // Sorted list of candidate preferences
}

/**
 * Interface representing the results of a single IRV round
 */
interface RoundResult {
  round: number;                                    // Round number (1, 2, 3, ...)
  counts: Record<string, number>;                   // Vote counts per candidate (candidateId -> count)
  eliminated: string | null;                        // ID of eliminated candidate (null if no elimination)
  winner: string | null;                            // ID of round winner (null if no winner yet)
}

/**
 * Interface representing the final tally result
 */
interface TallyResult {
  winnerIds: string[];   // Array of winner candidate IDs
  rounds: RoundResult[]; // Array of all round results
}
 ...
 */
export async function runTally(pollID: string): Promise<TallyResult> {
  ...
  const revealedVotes = await prisma.revealedVote.findMany({
    where: { pollId },
    select: {
      id: true,              // Vote ID
  ...
  if (revealedVotes.length === 0) {
    const result: TallyResult = { winnerIds: [], rounds: [] };
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.tallyResult.create({
        data: {
          id: generateSnowflakeIdBigInt(),     // Generate unique snowflake ID
          pollId,                                // Link to poll
          resultsData: { winners: [], rounds: [] } as any, // Empty results (cast to any for Prisma Json)
  ...
  const ballots: VoteRanking[] = revealedVotes.map((v) => {
    // Decrypt returns object: { candidateId: BigInt, rank: number }[]
    const rankingsRaw = decryptRankings(v.rankings as string);
  ...
  while (activeCandidateIds.size > 0) {
    ...
    if (!roundWinner && activeCandidateIds.size === 1) {
      roundWinner = Array.from(activeCandidateIds)[0] as string;
    }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.tallyResult.create({
      data: {
        id: generateSnowflakeIdBigInt(),     // Unique ID for tally result
        pollId,                                // Link to poll
        resultsData: {
