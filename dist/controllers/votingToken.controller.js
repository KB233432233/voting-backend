...
exports.createVotingToken = createVotingToken;
exports.getVotingTokens = getVotingTokens;
exports.getVotingToken = getVotingToken;
exports.updateVotingToken = updateVotingToken;
exports.deleteVotingToken = deleteVotingToken;
...
const createTokenSchema = zod_1.z.object({
    userId: zod_1.z.string().regex(/^\d+$/, "User ID must be numeric"),
    pollId: zod_1.z.string().regex(/^\d+$/, "Poll ID must be numeric"),
});
const updateTokenSchema = zod_1.z.object({
    status: zod_1.z.enum(["active", "used", "expired"]),
});
const querySchema = zod_1.z.object({
    page: zod_1.z.coerce.number().int().positive().default(1),
    limit: zod_1.z.coerce.number().int().positive().max(100).default(20),
    status: zod_1.z.enum(["active", "used", "expired"]).optional(),
    userId: zod_1.z.string().regex(/^\d+$/).optional(),
    pollId: zod_1.z.string().regex(/^\d+$/).optional(),
});
// Helper: Mask token hash for secure logging/display
const maskHash = (hash) => `${hash.slice(0, 8)}...${hash.slice(-4)}`;
/**
 * POST /voting-tokens
 * Issue a new voting token (admin/issuance endpoint)
 */
async function createVotingToken(req, res) {
    try {
        ...
        const [user, poll] = await Promise.all([
            db_ts_1.default.user.findUnique({ where: { id: userId } }),
            db_ts_1.default.poll.findUnique({ where: { id: pollId } }),
        ]);
        if (!user || !poll) {
        ...
        const existing = await db_ts_1.default.votingToken.findFirst({
        ...
        const tokenHash = crypto_1.default.createHash("sha256").update(tokenBuffer).digest("hex");
        const token = await db_ts_1.default.votingToken.create({
            data: {
 ...
 */
async function getVotingTokens(req, res) {
    try {
        ...
        const [tokens, total] = await db_ts_1.default.$transaction([
            db_ts_1.default.votingToken.findMany({ where, orderBy: { issuedAt: "desc" }, skip, take: limit }),
            db_ts_1.default.votingToken.count({ where }),
        ]);
        res.status(200).json({
            success: true,
            tokens: tokens.map((t) => ({
 ...
 * Get single token
 */
async function getVotingToken(req, res) {
    try {
        ...
        const token = await db_ts_1.default.votingToken.findUnique({ where: { id } });
        if (!token) {
 ...
 */
async function updateVotingToken(req, res) {
    try {
        const idParam = req.params.id;
        if (!idParam) {
            res.status(400).json({ error: "Invalid token ID" });
        ...
        const updated = await db_ts_1.default.votingToken.update({
            where: { id },
            data: {
                status,
 ...
 * Revoke/delete token
 */
async function deleteVotingToken(req, res) {
    try {
        const idParam = req.params.id;
        if (!idParam) {
            res.status(400).json({ error: "Invalid token ID" });
        ...
        await db_ts_1.default.votingToken.delete({ where: { id } });
...
//# sourceMappingURL=votingToken.controller.js.map