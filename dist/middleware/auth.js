...
const redis_scripts_ts_1 = require("../utils/redis-scripts.ts");
const jwt_ts_1 = require("../utils/jwt.ts");
...
async function createNonce(address) {
    ...
    await redis.set(`nonce:${nonce}`, address.toLowerCase(), // <-- just the address
    "EX", ttlInSeconds);
 ...
 * ```
 */
async function validateNonce(nonce, address) {
    ...
    try {
        ...
        const result = await redis.eval(redis_scripts_ts_1.VALIDATE_NONCE_SCRIPT, 1, // number of keys
...
function requireAuth(req, res, next) {
    ...
    try {
        ...
        const payload = (0, jwt_ts_1.verifyAccessToken)(token, JWT_SECRET);
